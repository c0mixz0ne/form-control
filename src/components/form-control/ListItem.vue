...
<template>
  <div class="list-item">
    <n-input
      @blur="updateHandlerLabel(account, index)"
      v-model:value="label"
      type="text"
      size="medium"
      placeholder="Метки"
    ...
    />
    <n-select
      :status="errors[index]?.type ? 'error' : ('success' as ErrorStatus)"
      @vue:updated="updateHandler(account, index)"
    ...
    />
    <n-input
      :status="errors[index]?.login ? 'error' : ('success' as ErrorStatus)"
      @blur="updateHandler(account, index)"
    ...
    />
    <n-input
      :status="errors[index]?.password ? 'error' : ('success' as ErrorStatus)"
      @blur="updateHandler(account, index)"
      v-model:value="account.password"
      v-if="account.type === 'Локальная' || account.type === null"
      type="password"
